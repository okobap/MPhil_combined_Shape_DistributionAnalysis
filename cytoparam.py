...
def parameterize_image_coordinates_fromMesh(
    mesh_mem, mesh_nuc, lmax: int, nisos: List
):
    """
    Runs the parameterization for a cell represented by its spherical
    harmonics coefficients calculated by using ther package aics-shparam.
    ...
    # Cell SHE coefficients
    x,y,coeffs_mem = getshcoefficients(mesh_mem.vertices, lmax)
    centroid_mem = [0,0,0]

    # Nuclear SHE coefficients
    x,y, coeffs_nuc = getshcoefficients(mesh_mem.vertices, lmax)
    centroid_nuc = [0,0,0]
...
def cellular_mapping(
    ...
    for i, iso_value in enumerate(np.linspace(0.0, 1.0, 1 + np.sum(nisos))):
        ...
        isosurfacesMesh.append(vedo.Mesh(mesh, alpha= 0.5))
        
   # vedo.show(isosurfacesMesh[32],isosurfacesMesh[64],  __doc__, viewup='z', axes=1).close()
...
def get_intensity_representation(polydata: vtk.vtkPolyData, images_to_probe: List):
    ...
    representation = {}
    coords = vtkmodules.util.numpy_support.vtk_to_numpy(polydata.GetPoints().GetData())
    # print(coords)
